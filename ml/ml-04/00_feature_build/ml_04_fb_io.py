"""I/O helpers for ML-04 feature build.
...
- Notes: DEFAULT_INPUT_PATH is the approved ML-03 r00 all parquet contract.
"""
...
from ml_04_fb_utils import BASE_DIR


DEFAULT_INPUT_PATH = BASE_DIR / "ml" / "ml-03" / "fb_outputs" / "r00" / "ml_03__r00_Xy_all.parquet"


def _parquet_module():
    """Import pyarrow.parquet only when a parquet helper actually needs it."""

    try:
        import pyarrow.parquet as pq
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "pyarrow is required for parquet file I/O. "
            "Install project requirements or use build_features_from_frame() with an in-memory DataFrame."
        ) from exc
    return pq
...
def load_parquet_split_sample(
    ...
    *,
    ...
    keeps at most ``sample_rows // len(split_values)`` rows per split. The
    requested sample size must be large enough to include one row per split.
    """
    ...
    if sample_rows < len(split_values):
        raise ValueError(
            "sample_rows is too small for split-aware sampling. "
            f"sample_rows={sample_rows}, required_minimum={len(split_values)}, split_values={list(split_values)}"
        )
