"""ML-04 feature build entry points.

This module reads a split-aware ML-03 feature parquet or DataFrame, validates
the existing split, and appends only ML-04 Stage 3 sender-receiver relationship
features.
It does not save files directly; export is handled by ``ml_04_fb_encoding``.

Code map:
- Input: ML-03 split-aware parquet or in-memory DataFrame.
...
import ml_04_fb_build_validation as build_validation
from ml_04_fb_build_artifacts import assemble_build_artifacts, preserve_source_columns
from ml_04_fb_io import DEFAULT_INPUT_PATH, load_parquet_columns, load_parquet_split_sample, parquet_columns, resolve_path
from ml_04_fb_operations import SUPPORTED_BATCH_OPERATIONS
from ml_04_fb_schema import resolve_requested_columns, standardize_input_frame, validate_no_forbidden_input_columns
from ml_04_fb_specs import FeatureSpec, ml04_stage3_feature_specs, required_input_columns, validate_feature_specs
...
class FeatureBuildConfig:
    ...
    experiment_id: str = "ML-04"
    run_name: str = "stage3_pair_relationship_full45_ratio_stability"
...
def build_features(config: Optional[FeatureBuildConfig] = None) -> FeatureBuildResult:
    """Build ML-04 features from a split-aware parquet input without saving files."""
...
def build_features_from_frame(
    ...
    *,
    ...
    """Build ML-04 features from an in-memory split-aware DataFrame without saving files."""
...
def _validate_specs_for_build(specs: Tuple[FeatureSpec, ...]) -> None:
    ...
    if unsupported_specs:
        raise ValueError(
            "Feature build failed: unsupported ML-04 feature operation. "
...
def _require_feature_specs(
    ...
    """Return explicit specs or the fixed ML-04 Stage 3 specs."""

    if feature_specs is None:
        return ml04_stage3_feature_specs()
    ...
    *,
    ...
    if "split" not in source_with_meta.columns:
        raise ValueError(
            ...
            "This ML-04 path does not create a new train/val/test split. "
            f"input={input_label}"
        )
...
def _build_from_split_frame(
    ...
    *,
    ...
    """Compute ML-04 features from a validated split frame."""
