"""Feature catalog and split summary builders for ML-03.
...
from ml_03_fb_schema import validate_no_forbidden_feature_columns
from ml_03_fb_specs import FeatureSpec, feature_columns, validate_feature_specs


FEATURE_SET_LABEL = "full46"
...
def _selection_status(spec: FeatureSpec) -> str:
    if "tx_count__degree" in spec.output_col:
        return f"candidate_{FEATURE_SET_LABEL};duplicate_check_required"
...
def make_feature_catalog(
    ...
    *,
    ...
    """Convert selected FeatureSpecs to a reviewable ML-03 feature catalog."""
        ...
        rows.append(
            {
                ...
                "leakage_rule": "Use only rows with history_timestamp < current_timestamp; lower window bound inclusive.",
                "leakage_check": "Operation computes timestamp groups before adding same-timestamp rows to history.",
