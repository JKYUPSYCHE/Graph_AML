...
def validate_split_column(df: pd.DataFrame, expected_split: str | None, source_path: str | Path) -> None:
    ...
    if expected_split is None or "split" not in df.columns:
        return
    values = set(df["split"].dropna().astype(str).str.lower().unique().tolist())
...
def load_split(
    ...
    if expected_split is not None and "split" in available_columns and "split" not in read_columns:
