"""CPU/GPU acceleration policy for ML common XGBoost runs.
...
def get_xgboost_version() -> str:
    ...
    try:
        ...
        raise XGBAccelerationError(
            "xgboost is required for ML common training. Install requirements.txt first."
        ) from exc
...
def require_expected_xgboost_version(version: str) -> None:
    ...
    if version != EXPECTED_XGBOOST_VERSION:
        raise XGBAccelerationError(
            "Unsupported xgboost version for ML common accelerator policy. "
