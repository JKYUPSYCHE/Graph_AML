"""Feature catalog and split summary builders for ML-05.
...
from ml_05_fb_specs import FeatureSpec, feature_columns, validate_feature_specs


FEATURE_SET_LABEL = "full48_flowbalance_passflow"
...
def _selection_status(spec: FeatureSpec) -> str:
    if "balanced_state_flag" in spec.output_col:
        return f"candidate_{FEATURE_SET_LABEL};train_quantile_threshold;no_history_set_to_0"
    if "amount__in_out_ratio" in spec.output_col or "current_to_last_in_amount_ratio" in spec.output_col:
        return f"candidate_{FEATURE_SET_LABEL};denominator_zero_handled_as_0"
    if "sequence_count" in spec.output_col:
