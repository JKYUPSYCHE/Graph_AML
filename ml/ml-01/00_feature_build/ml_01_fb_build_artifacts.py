"""ML-01 feature build artifact 조립/저장 helper."""
...
from ml_01_fb_build_validation import validate_stage0_rolling_outputs
...
from ml_01_fb_specs import FeatureSpec, feature_columns
...
def assemble_build_artifacts(
    ...
    *,
    ...
    """계산 결과, 검토용 테이블, summary를 저장 직전 형태로 조립한다."""
    ...
    build_summary["stage0_rolling_validation"] = validate_stage0_if_present(feature_frame)
    ...
    return BuildArtifacts(
        ...
        val_df=val_df,
        test_df=test_df,
        feature_columns_table=feature_columns_table,
...
def save_build_artifacts(paths: FeatureBuildOutputPaths, artifacts: BuildArtifacts) -> dict[str, Any]:
    """검증과 계산이 끝난 build artifact를 디스크에 저장한다."""

    build_summary = build_summary_with_outputs(artifacts.build_summary, paths)
    paths.output_dir.mkdir(parents=True, exist_ok=True)
    save_dataframe_parquet(artifacts.train_df, paths.train_path)
    save_dataframe_parquet(artifacts.val_df, paths.val_path)
    save_dataframe_parquet(artifacts.test_df, paths.test_path)
    save_dataframe_csv(artifacts.feature_columns_table, paths.feature_columns_path)
    save_dataframe_csv(artifacts.feature_catalog, paths.feature_catalog_path)
    save_dataframe_csv(artifacts.split_summary, paths.split_summary_path)
    save_dataframe_csv(artifacts.feature_info, paths.feature_info_path)
    save_dataframe_csv(artifacts.operation_artifacts["category_mapping"], paths.category_mapping_path)
    save_dataframe_csv(artifacts.operation_artifacts["category_unknown_summary"], paths.category_unknown_summary_path)
    save_json(build_summary, paths.build_summary_path)
    return build_summary


def build_summary_with_outputs(summary: Mapping[str, Any], paths: FeatureBuildOutputPaths) -> dict[str, Any]:
    """저장 완료 후 JSON에 기록할 outputs 섹션을 추가한 summary 복사본을 만든다."""

    output_summary = dict(summary)
    output_summary["outputs"] = build_output_paths_payload(paths)
    return output_summary


def build_output_paths_payload(paths: FeatureBuildOutputPaths) -> dict[str, str]:
    """build_summary의 outputs 섹션을 저장 경로 객체에서 만든다."""
    ...
    return {
        ...
        "feature_catalog_path": str(paths.feature_catalog_path),
        "split_summary_path": str(paths.split_summary_path),
        "feature_info_path": str(paths.feature_info_path),
...
def validate_stage0_if_present(feature_frame: pd.DataFrame) -> dict[str, Any]:
    """Stage 0 rolling 컬럼이 있으면 저장 전 semantic validation을 실행한다."""

    has_stage0_rolling = any(
        column.startswith("timehist__") and ("__tx_count__count__" in column or "__amount__sum__" in column)
        for column in feature_frame.columns
    )
    if not has_stage0_rolling:
        return {"status": "skipped", "reason": "stage0_rolling_columns_not_present"}
    result = validate_stage0_rolling_outputs(feature_frame)
