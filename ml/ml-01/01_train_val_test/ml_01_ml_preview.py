...
2. 데이터 흐름 요약
- 입력: ml_00_ml_io.InputPaths, split_summary_path, feature_columns_path
...
# ML 입력 경로 해석, feature CSV 검증, parquet schema 확인 등 공통 입출력 검증 규칙은 ml_00_ml_io 모듈에 위임한다.
# 확인 필요: ml_00_ml_io 내부에서 project_root, feature mask, label_col을 어떤 기준으로 검증하는지는 이 코드만으로는 확정할 수 없다.
import ml_01_ml_io
...
def _resolve_file(
    ...
    """
    # 경로 해석 정책을 이 함수에서 직접 구현하지 않고 ml_00_ml_io에 위임한다.
    resolved = ml_01_ml_io.resolve_project_path(path, project_root)
...
def load_selected_feature_rows(
    ...
    *,
    ...
    # feature CSV의 기본 계약 검증은 ml_00_ml_io.check_feature_columns_file에 위임한다.
    feature_check = ml_01_ml_io.check_feature_columns_file(
    ...
    selected_mask = ml_01_ml_io.parse_used_in_ml(feature_table["used_in_ml"])
    ...
    # 화면에 표시할 feature 순서와 ml_00_ml_io가 검증한 selected_columns 순서가 같은지 확인한다.
    ...
    if displayed_columns != feature_check.selected_columns:
        raise ValueError(
            "Displayed feature columns do not match ml_00_ml_io selected columns. "
...
def validate_split_schemas(
    ...
    *,
    ...
    for split_name, split_path in split_paths.items():
        ...
        if split_path is None:
            continue
        schema_names = set(ml_01_ml_io.get_parquet_columns(split_path)) # parquet 컬럼명만 읽어 schema 검증을 수행한다.
...
def preview_ml_inputs(
    ...
    *,
    ...
    # 확인 필요: InputPaths가 정확히 어떤 필드를 가지는지는 ml_00_ml_io.InputPaths 정의 확인이 필요하다.
    ml_01_ml_io.require_input_files(paths, require_test=True)
    ...
    return MLInputPreview(
        ...
        feature_columns_hash=ml_01_ml_io.feature_columns_hash(feature_columns),
