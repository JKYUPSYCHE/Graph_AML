"""ML-01 Optuna tuning pipeline for XGBoost.
...
from ml_01_ml_io import InputPaths, preflight_ml_inputs, resolve_project_path, save_json
from ml_01_ml_optuna_spaces import suggest_xgb_params
from ml_01_ml_train import XGBTrainConfig, XGBTrainResult, train_xgb
from ml_01_ml_val import ValidationConfig, ValidationResult, validate_xgb
...
def _require_optuna() -> Any:
    try:
        ...
        raise MissingOptionalDependencyError(
            "Optuna is required for ML-01 tuning. Install dependencies from requirements.txt."
        ) from exc
    return optuna
...
def _run_study(
    ...
    *,
    ...
    study = optuna.create_study(direction="maximize", sampler=sampler, study_name=f"ml01_{stage_name}")
...
def run_ml01_optuna_pipeline(config: OptunaPipelineConfig) -> OptunaPipelineResult:
    """Run smoke tuning, full tuning, and final train/validation for ML-01."""
