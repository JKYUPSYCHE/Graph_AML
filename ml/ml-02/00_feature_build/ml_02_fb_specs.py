...
실제 계산은 ml_02_fb_operations.py와 ml_02_fb_rolling.py에서 수행한다.
...
2. FeatureOpResult
   ...
   - 실제 계산 결과는 ml_02_fb_operations.py에서 생성된다.
3. *_spec helper 함수
...
예:
- ML-02: Stage 1 account statistics feature spec 생성
...
# 왜 상수로 박아두는가
# --------------------
# - ml_02_fb_operations.execute_feature_specs()가 최종 feature_frame을 조립할 때 [META_COLUMNS] + [생성 feature columns] 순서로 컬럼을 배치.
...
class FeatureSpec:
    ...
    주요 필드
    ---------
    operation:  실행할 연산 이름. `ml_02_fb_operations.OPERATION_REGISTRY`에 등록되어 있어야 한다.
    ...
    # 이 문자열이 ml_02_fb_operations.OPERATION_REGISTRY의 key와 정확히 매치돼야 한다.
    # 등록되지 않은 이름이면 ml_02_fb_operations.run_operation()에서 즉시 에러.
    operation: str
    ...
    # 각 operation 함수가 require_allowed_params()로 허용 키를 검사하므로,
...
def validate_feature_specs(feature_specs: Tuple[FeatureSpec, ...]) -> None:
    ...
    위 항목들은 `ml_02_fb_operations.py`의 operation 검증,
...
DEFAULT_ACCOUNTSTATS_AMOUNT_WINDOWS: Tuple[Tuple[str, str], ...] = (
    ("w1d", "1d"),
    ("w3d", "3d"),
    ("w7d", "7d"),
)

DEFAULT_ACCOUNTSTATS_FLOW_WINDOWS: Tuple[Tuple[str, str], ...] = (
    ("w6h", "6h"),
    ("w1d", "1d"),
    ("w3d", "3d"),
    ("w7d", "7d"),
...
def rolling_entity_agg_spec(
    ...
    *,
    ...
    agg: str,
    fill_value: float = 0.0,
    family: str = "accountstats",
    description: str = "Past-only rolling aggregation for a current-row account using a directional history entity.",
    aml_typology: str = "account_flow_history",
    used_in_ml: bool = True,
) -> FeatureSpec:
    """현재 row 계좌와 과거 row의 방향별 계좌를 매칭하는 rolling aggregation 선언을 만든다."""

    agg = _clean_name(agg, "rolling entity agg").lower()
    if agg != "sum":
        raise ValueError(f"Unsupported rolling entity agg: {agg!r}")
    return FeatureSpec(
        operation="rolling_entity_agg",
        output_col=output_col,
        input_cols={
        ...
        entity_scope="account",
        direction="past_window",
        leakage_policy="past-only; history rows require timestamp < current timestamp; same-timestamp rows excluded",
        computational_cost="medium",
        used_in_ml=used_in_ml,
    )


# Retained for external FeatureSpec compatibility and future ablation runs.
# ml02_stage1_feature_specs() does not use these Stage 0/time-history helpers.
...
def ml02_stage1_feature_specs(*, used_in_ml: bool = True) -> Tuple[FeatureSpec, ...]:
    """보완안 기준 ML-02 Stage 1 계좌별 통계 feature 선언을 반환한다.

    ML-01 입력 parquet에 이미 포함된 Stage 0 피처는 여기서 다시 만들지 않는다.
    이 함수는 기존 컬럼을 보존한 상태에서 Stage 1 신규 계좌별 통계 컬럼만 추가한다.
    """

    specs: list[FeatureSpec] = []

    for suffix, window in DEFAULT_ACCOUNTSTATS_AMOUNT_WINDOWS:
        suffix = _clean_name(suffix, "window suffix")
        window = _clean_name(window, "window")
        for agg in ("mean", "std"):
            specs.append(
                rolling_agg_spec(
                    ...
                    "timestamp",
                    "amount",
                    f"accountstats__sender__out__amount__{agg}__{suffix}",
                    window=window,
                    agg=agg,
                    family="accountstats",
                    description=f"Sender account past outgoing amount {agg} over {window}.",
                    aml_typology="amount_variability",
                    used_in_ml=used_in_ml,
                )
            )
            specs.append(
                rolling_agg_spec(
                    ...
                    "timestamp",
                    "amount_received",
                    f"accountstats__receiver__in__amount__{agg}__{suffix}",
                    window=window,
                    agg=agg,
                    family="accountstats",
                    description=f"Receiver account past incoming amount {agg} over {window}.",
                    aml_typology="amount_variability",
                    used_in_ml=used_in_ml,
                )
            )
    ...
    flow_specs = (
        ("sender", "out", "sender_account_id", "sender_account_id", "amount"),
        ("sender", "in", "sender_account_id", "receiver_account_id", "amount_received"),
        ("receiver", "in", "receiver_account_id", "receiver_account_id", "amount_received"),
        ("receiver", "out", "receiver_account_id", "sender_account_id", "amount"),
    )
    for role, direction, current_entity_col, history_entity_col, value_col in flow_specs:
        for suffix, window in DEFAULT_ACCOUNTSTATS_FLOW_WINDOWS:
            suffix = _clean_name(suffix, "window suffix")
            window = _clean_name(window, "window")
            specs.append(
                rolling_entity_agg_spec(
                    current_entity_col,
                    history_entity_col,
                    "timestamp",
                    value_col,
                    f"accountstats__{role}__{direction}__amount__sum__{suffix}",
                    window=window,
                    agg="sum",
                    family="accountstats",
                    description=f"{role.capitalize()} account past {direction} amount sum over {window}.",
                    aml_typology="in_out_amount_history",
                    used_in_ml=used_in_ml,
                )
            )
