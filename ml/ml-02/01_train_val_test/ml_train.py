...
import joblib
from ml_cpugpu import XGBAccelerationConfig, resolve_xgb_acceleration
from ml_utils import set_seed

# ml_io는 이전 단계에서 만든 입출력/검증 유틸리티 모듈이다. 이 학습 모듈은 parquet를 직접 읽지 않고, load_split()에 위임한다.
# 변경 영향:
# - load_feature_columns()의 feature 선택 규칙이 바뀌면 모델 입력 컬럼 전체가 바뀐다.
# - load_split()의 결측치/타입/expected_split 검증 규칙이 바뀌면 학습 가능 데이터가 달라진다.
# 확인 필요: 각 함수의 세부 검증 기준은 ml_io 구현을 확인해야 한다.
from ml_io import (
    InputPaths,
...
# ml_resource는 학습 시간, 메모리, 데이터 프로파일, 환경 정보, XGBoost 진단 정보, feature importance 저장을 담당한다.
...
class XGBTrainConfig:
    ...
    accelerator: str = "auto"               # auto/cpu/cuda. 실제 XGBoost tree_method는 ml_cpugpu에서 중앙 관리.
    ...
    def __post_init__(self) -> None:
        ...
        if self.feature_assoc_train_sample_rows is not None and self.feature_assoc_train_sample_rows <= 0:
            raise ValueError("feature_assoc_train_sample_rows must be a positive integer or None.")
...
def require_training_files(config: XGBTrainConfig) -> None:
    """
    학습에 필요한 입력 파일, parquet schema, encoding manifest 정합성을 확인한다.
    입력 파일
    --------
    - train parquet, validation parquet, feature columns CSV, 선택적 encoding manifest
    변경 영향
    --------
    - 학습 산출물을 만들기 전에 승인 입력 불일치와 native categorical manifest 누락을 먼저 차단한다.
    """
    preflight_ml_inputs(
        InputPaths(
...
def build_xgb_model(
    ...
    - tree_method/predictor: ml_cpugpu에서 CPU/GPU 정책에 맞춰 결정
    ...
    # CPU/GPU별 XGBoost 파라미터는 ml_cpugpu에서만 결정한다.
...
def _prepare_train(config: XGBTrainConfig) -> str | None:
    """Seed runtime, validate inputs, and protect existing train artifacts."""
    ...
    return feature_assoc_file_name
...
def _load_train_inputs(
    *,
    ...
    """Load feature contract plus train/validation splits."""
...
def _run_training(
    *,
    ...
    with runtime_tracker.measure("build_model"):
        scale_pos_weight = compute_scale_pos_weight(train_inputs["y_train"])
        acceleration = resolve_xgb_acceleration(config.accelerator)
        model = build_xgb_model(
            config,
            scale_pos_weight=scale_pos_weight,
            acceleration=acceleration,
            enable_categorical=bool(train_inputs["categorical_feature_columns"]),
        )
    ...
    with runtime_tracker.measure("fit"):
        model.fit(
            ...
            eval_set=[
                (train_inputs["x_train"], train_inputs["y_train"]),
                (train_inputs["x_val"], train_inputs["y_val"]),
            ],
            verbose=False,
        )

    training_time_sec = runtime_tracker.as_dict().get("fit", 0.0)
    memory_tracker.snapshot("after_fit")
...
def _build_train_outputs(
    *,
    ...
    """Collect metadata used by train_summary.json."""
    ...
    with runtime_tracker.measure("build_metadata"):
        ...
        data_profile = make_data_profile(
            {
                "train": (config.train_path, train_inputs["x_train"], train_inputs["y_train"]),
                "val": (config.val_path, train_inputs["x_val"], train_inputs["y_val"]),
            },
            train_inputs["feature_columns"],
        )
...
def _save_train_outputs(
    *,
    ...
    """Save model, feature order, and feature importance artifacts."""
    ...
    with runtime_tracker.measure("save_artifacts"):
        ...
        save_feature_columns(train_inputs["feature_columns"], feature_columns_out)
        feature_importance_frame = save_feature_importance(
...
def train_xgb(config: XGBTrainConfig) -> XGBTrainResult:
    ...
    try:
        with runtime_tracker.measure("prepare_inputs"):
            feature_assoc_file_name = _prepare_train(config)

        train_inputs = _load_train_inputs(
            config=config,
            runtime_tracker=runtime_tracker,
            memory_tracker=memory_tracker,
        )
        feature_columns = train_inputs["feature_columns"]
        features_hash = train_inputs["features_hash"]
        categorical_feature_columns = train_inputs["categorical_feature_columns"]
        x_train = train_inputs["x_train"]
        y_train = train_inputs["y_train"]
        x_val = train_inputs["x_val"]
        y_val = train_inputs["y_val"]
        ...
        model = training_result["model"]
        scale_pos_weight = training_result["scale_pos_weight"]
        acceleration = training_result["acceleration"]
        training_time_sec = training_result["training_time_sec"]

        output_metadata = _build_train_outputs(
            config=config,
            train_inputs=train_inputs,
            training_result=training_result,
            runtime_tracker=runtime_tracker,
        )
        best_iteration = output_metadata["best_iteration"]
        best_score = output_metadata["best_score"]
        n_estimators_ceiling_hit = output_metadata["n_estimators_ceiling_hit"]
        data_profile = output_metadata["data_profile"]
        xgboost_diagnostics = output_metadata["xgboost_diagnostics"]
        learning_curve = output_metadata["learning_curve"]
        environment = output_metadata["environment"]

        saved_outputs = _save_train_outputs(
            config=config,
            train_inputs=train_inputs,
            training_result=training_result,
            runtime_tracker=runtime_tracker,
        )
        model_path = saved_outputs["model_path"]
        feature_columns_out = saved_outputs["feature_columns_out"]
        train_summary_path = saved_outputs["train_summary_path"]
        scores_train_summary_path = saved_outputs["scores_train_summary_path"]
        feature_importance_path = saved_outputs["feature_importance_path"]
        feature_assoc_file_name = saved_outputs["feature_assoc_file_name"]
        feature_assoc_path = saved_outputs["feature_assoc_path"]
        feature_importance_frame = saved_outputs["feature_importance_frame"]
        model_sha256 = saved_outputs["model_sha256"]
        feature_columns_file_sha256 = saved_outputs["feature_columns_file_sha256"]
        feature_importance_file_sha256 = saved_outputs["feature_importance_file_sha256"]
    ...
    if config.export_feature_assoc:
        with runtime_tracker.measure("export_feature_assoc_train"):
            feature_assoc_payload = make_mixed_feature_association_payload(
                ...
                sample_strategy=(
                    "full"
                    if config.feature_assoc_train_sample_rows is None
    ...
    train_summary = {
        ...
        "run_metadata": make_run_metadata(
            config.output_dir,
            seed=config.seed,
            artifact_file_name=config.model_file_name,
        ),
        ...
        # 데이터, 실행 환경, XGBoost 내부 상태 진단 정보다. 정확한 세부 필드는 ml_resource 구현 확인 필요.
        "data_profile": data_profile,
        "environment": environment,
        "xgboost_diagnostics": xgboost_diagnostics,
