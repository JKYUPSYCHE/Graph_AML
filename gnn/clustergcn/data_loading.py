...
def get_data(args, data_config):
    '''Loads AML transaction data. Always returns homo GraphData (HeteroData conversion
    is done on-the-fly per batch in train_util.py for ClusterGCN compatibility).'''
    ...
    # NOTE: create_hetero_obj는 여기서 호출하지 않음.
    # reverse_mp=True일 때 HeteroData 변환은 학습 루프에서 배치 단위로 수행 (homo_to_hetero).
